...
const Assign = () => {
	...
	const fetchMarks = async () => {
		...
		const response = await axios.post(
			'http://localhost:5000/api/student/getMarks',
	...
	const saveChanges = async () => {
		const response = await axios.post(
			'http://localhost:5000/api/student/editMarks',
			{
				student_id: student_id,
				viva: viva,
				ideation: ideation,
	...
	useEffect(() => {
		fetchMarks();
		const l = parseInt(localStorage.getItem('mentor_locked'));
		console.log(l);
		setLocked(l);
	}, []);
						...
						<div className='grid grid-flow-col mt-10 sm:auto-cols-max justify-start sm:justify-end gap-2'>
							<div className='flex flex-col'>
								...
								<div className='mt-6'>
									<button
										onClick={saveChanges}
										class='inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-green-600 border rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'
									...
									<button
										onClick={goBack}
										class='inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-red-600 border rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'
										data-rounded='rounded-md'
										data-primary='blue-600'
										data-primary-reset='{}'
									>
									...
									{locked === 1 ? (
										<button
											disabled
											class='inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-gray-600 border rounded-md shadow-sm hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500'
											data-rounded='rounded-md'
											data-primary='blue-600'
											data-primary-reset='{}'
										>
