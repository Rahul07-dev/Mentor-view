...
const Dashboard = () => {
	...
	const fetchStudents = async () => {
		...
		const response = await axios.post(
			'http://localhost:5000/api/mentor/get_assigned_students',
			{ mentor_id: mentor_id }
		);
	...
	const addNewStudents = async () => {
		...
		if (len >= 3 && len <= 4) {
			...
			const response = await axios.post(
				'http://localhost:5000/api/mentor/add_students',
				{ student_list: assignNewStudents, mentor_id: mentor_id }
			);
		} else {
	...
	const replaceStudentFunc = async () => {
		fetchStudents();
		const response = await axios.post(
			'http://localhost:5000/api/mentor/rep_student',
			{ ns_id: replacableStudentId, ds_id: deleteableStudent, m_id: mentor_id }
		);
		// if (response)
		// {
	...
	const deleteStudent = async (s_id) => {
		...
		if (len == 3) {
			...
			const response = await axios.post(
				'http://localhost:5000/api/mentor/del_student',
				{ s_id: s_id }
			);
			fetchStudents();
			fetchUnassignedStudents();
	...
	const fetchUnassignedStudents = async () => {
		const response = await axios.get(
			'http://localhost:5000/api/mentor/get_unassigned_students'
		);
		const arr = response.data.students;
		// console.log(arr);
	...
	const getMentor = async () => {
		const response = await axios.get(
			`http://localhost:5000/api/mentor/currentmentor/${mentor_id}`
		);
	...
	const lock = async () => {
		...
		const response = await axios.post(
			'http://localhost:5000/api/mentor/locked',
			{ m_id: mentor_id },
			config
		);
	...
	const handleLock = async () => {
		try {
			if (
				marksNotAssignedStudents.length === 0 &&
				marksAssignedStudents.length >= 3 &&
				marksAssignedStudents.length <= 4
			) {
				lock();
	...
	useEffect(() => {
		localStorage.setItem('mentor_locked', mentor.locked);
		localStorage.setItem('mentor_id', mentor.m_id);
	}, [mentor]);
				...
				<section className='px-4 relative bg-[#F1F5F9] dark:bg-[#0c0a09] sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto'>
					<div className='sm:flex sm:justify-between sm:items-center mb-8'>
						...
						{mentor.locked === 1 ? (
							<button
								disabled
								class='inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-gray-600 border border-gray-700 rounded-md shadow-sm hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500'
										...
										<tbody className='text-[13px] '>
											<tr className='border-[#151515]'>
												<td className='px-10 border-r-2 border-zinc-600  first:pl-5 last:pr-5 py-4 whitespace-nowrap'>
													<div className='flex justify-center text-lg font-medium items-center'>
														{student.s_id}
													</div>
												</td>
												<td className='px-10 border-r-2 border-zinc-600  first:pl-5 last:pr-5 py-4 whitespace-nowrap'>
					...
					<div className='mt-4 flex justify-center items-center'>
						{mentor.locked === 1 ? (
							<h3>Students and marks are locked!!!</h3>
						) : (
							<button
								onClick={() => handleLock()}
								class='inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-blue-600 border border-blue-700 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'
						...
						<div className='relative 2xl:mt-0 mt-36 2xl:mb-0 md:mt-36 mb-8 mx-auto 2xl:w-[600px] md:w-[600px] w-[300px]'>
							<div className='border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none'>
								<div className='flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t'>
									<h3 className='text-xl font-semibold'>Add Student Form</h3>
								</div>
